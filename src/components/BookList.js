...
const BookList = () => {
  ...
  // Function to apply the filters to the book data
  const filteredBooks = books.filter((book) => {
    return (
      ...
      (filter.published ? book.published === parseInt(filter.published) : true)
    );
  });
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {filteredBooks.map((book) => (
          <div key={book.id} className="border p-4">
            <img src={book.imageUrl} alt={book.title} className="w-[500px] h-[500px] object-cover mb-4" />
            <h3 className="text-lg font-bold">{book.title}</h3>
            <p>Author: {book.author}</p>
            <p>Genre: {book.genre}</p>
            <p>Published: {book.published}</p>
            <Link to={`/book/${book.id}`} className="mt-2 bg-blue-500 text-white py-1 px-3 inline-block">View Details</Link>
          </div>
        ))}
      </div>
      ...
      {filteredBooks.length === 0 && (
        <p className="text-red-500 mt-4">No books found based on your filters.</p>
      )}
    </div>
  );
};
